...
pipeline {
  ...
  parameters {
    ...
    string(name: 'ImageName', description: "Name of the docker build", defaultValue: "online_books_store_deployment")
	string(name: 'ImageTag', description: "Name of the docker build",defaultValue: "v1")
	      string(name: 'AppName', description: "Name of the Application",defaultValue: "online_books_store_deployment")
    string(name: 'docker_repo', description: "Name of docker repository",defaultValue: "narina22")
  }
    ...
    stages {
        stage('Git-Checkout') {
            ...
            steps {
                gitCheckout(
                    branch: "main",
                    url: "https://github.com/saikrishnanarina/onlinebookstore.git"
                )
            }
        }
        stage('Build-Maven'){
